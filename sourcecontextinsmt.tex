...
differ from the phrases used in the non-context informed output. To this end,
we ask moses to verbosely output\footnote{This can be done using the
\texttt{-T} flag}, for each output sentence, which phrases from the
intermediate phrase-translation table are used. We then analyse how many phrase
pairs are unique to the context-informed system. These phrases thus do not
occur in the baseline and are indicative of the SMT decoder making a different
choice than it did for the baseline, due to the classifier information.
...
\hline
\textbf{Corpus} & \textbf{Language Pair} & \textbf{Configuration} & \textbf{Unique phrase-pairs} & \textbf{Unique source phrases} &
\hline
EMEA & English$\rightarrow$Spanish & l1r1 (monolithic/uni/replace) & $43.68\%$ & $40.15\%$ \\
Fryske Akademy & Frisian$\rightarrow$Dutch & l1r1 (monolithic/opt/replace) & $33.57\%$ & $32.50\%$ \\
IWSLT 2012 TED & Dutch$\rightarrow$English & l1r1 (monolithic/opt/replace) & $26.86\%$ & $20.07\%$ \\
IWSLT 2006 & Chinese$\rightarrow$English & l1r1 (monolithic/opt/replace) & $24.55\%$ & $12.49\%$ \\
\hline
\end{tabular}
...
\todo{TODO: complete and interpret table}
...
In this discussion we would like to focus on why the results of our study
...
in the training data. If this is so, then this implies that there is often a
source fragment $s'$ in the phrase-translation table that is the conjunction of
$s$ and $C$, and which maps to a $t'$ of which $t$ is at least a substring. It
is thus likely that the context information we try to explicitly model, is
often already \emph{implicitly} available to the decoder. This is provided that
the classifier and phrase-translation table are trained on the same parallel
corpus, as we consistently did. Rather than considering a source phrase in
context, the decoder may thus opt to choose to include the context as integral
part of the phrase. 
