...
\hline
\multicolumn{6}{|c|}{IWSLT 2005 -- Chinese$\rightarrow$English} \\
\hline
...
was used.  In this section we focus exclusively on the question whether context-informed systems are beneficial, and if so, with what context size; a comparison of the score weighting methods and classifier configurations is postponed until a later section.  For the current and most of our experiments, weights to the SMT decoder have been optimised (\emph{opt}) using MERT, on the baseline.
...
a smaller margin also on the JRC Acquis corpus (Table~\ref{tab:jrc250}). Both
...
parameter optimisation, we instead used a uniform (uni) distribution of weights
...
of different context sizes.  An enlargment of the context size seems to be
...
rigidly stuctured, predictable, and therefore easier to translate than corpora
...
(Table~\ref{tab:iwslt2006}), which shows lower scores. We dive further into
this hypothesis with a series of experiments on a Dutch-Frisian corpus. The
high similarity between these two germanic languages of The Netherlands makes
translation easier, and has been shown to achieve high scores
\citep{OERSETTER}. Table~\ref{tab:fa2} shows our findings. Although high scores
were obtained as expected, we were completely unable to exceed the
...
kind of optimisation step. The counterside is that such systems may therefore
...
Whilst we conducted parameter optimisation for the parameters of the SMT
decoder, we did not yet do so \emph{for the classifiers}. Yet, these too can be
optimised. This is only feasible for the classifier experts; recall that the
monolithic classifier necessarily has extra constraints imposed on it to
prevent translations that do not occur in the phrase-translation table. This
has the disadvantage of making it hard to run standard parameter optimisation
algorithms. 

The experiments conducted until now have not seen their classifiers optimised
in such a way, because this comes with a number of challenges. Ideally,
the optimisation of classifier parameters should not be independent of the
optimisation of the decoder parameters, and the classifier parameters should be
evaluated on the end-result, i.e. the full sentential translations according to
one or more MT metrics. This, however, is prohibitively expensive as it vastly
increases the parameter search space and thus the complexity of the problem. As
a concession for the sake of computability, classifier parameter optimisation
is considered independently and assessed through simpler cross-validation or
leave-one-out methods (depending on the number of instances in the classifier
expert). This still introduces a significant computational bottleneck.
