...
The bypass method effectively takes each sentence individually and ensures that
the entries in the phrase-translation table are explicitly tuned to the source
context using the output from the classifier(s). Each instance of a source
phrase will get a separate entry in the phrase-translation table, as opposed to
one source phrase applying to all instances in the test data. 
...
phrase under consideration. The result of this classification is a
distribution of translation options for that source phrase in the given
context, along with a classifier score for each option. After normalisation,
this score is $p(t|s,C)$. We now have two options for integrating this
in the score vector for the phrase pair:
...
The method of choice is applied and the data is entered into the altered
