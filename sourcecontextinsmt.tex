...
Table~\ref{tab:iwslt2006zhen} shows a clear positive result on the data set of
the IWSLT 2006 Evaluation Campaign, Chinese to English. A dataset from the same
source \footnote{Due to this dataset not being publicly available. Moreover, we
no longer having means of comparing the exact set used by \cite{Stroppa+07}, we
have to assume our training and test sets, whilst drawn from the same source,
are distinct subsets from the ones used by \cite{Stroppa+07}} was used in
\cite{Stroppa+07}, who also reported positive results.

\begin{table}
\begin{tabular}{|l|ccccc|}
\textbf{System} & \textsc{BLEU}  & \textsc{METEOR}  & \textsc{NIST}  & \textsc{WER}  & \textsc{PER}  \\ 
\hline
\multicolumn{7}{c}{IWSLT 2005 -- Chinese$\rightarrow$English} \\
...
was used. The weights to the SMT decoder have been optimised (\emph{opt}) using
MERT, on the baseline.
...
\textbf{System} & \textsc{BLEU}  & \textsc{METEOR}  & \textsc{NIST}  & \textsc{WER}  & \textsc{PER}  \\ 
\hline
\multicolumn{7}{c}{europarl250k-nl-en} \\
\hline 
...
\textbf{System} & \textsc{BLEU}  & \textsc{METEOR}  & \textsc{NIST}  & \textsc{WER}  & \textsc{PER}  \\ 
\hline
\multicolumn{7}{c}{IWSLT 2012 -- Dutch$\rightarrow$English} \\
\hline 
\hline 
...
The below-baseline results for Dutch-English on two corpora may suggest that
the efficacy of source-side context modelling depends strongly on the language
pair. A possible hypothesis here is that source-side context modelling stands to gain
most when translation from morphologically simpler languages to morphologically
richer languages, as the source-side context information may help in
determining the proper morphological form.  At the least, and more generally
put, we can posit that it depends a lot on the particular data used, as we note
a lot of variability in our experiments. Further experiments in this section
will underline this observation. 

%TODO: english-spanish





%TODO: english-russian
...
Table~\ref{tab:fa2} shows that for the Frisian-Dutch corpus,
we were completely unable to exceed the non-context-informed baseline. 
...
\textbf{System} & \textsc{BLEU}  & \textsc{METEOR}  & \textsc{NIST}  & \textsc{WER}  & \textsc{PER}  \\ 
\hline
\multicolumn{7}{c}{Fryske Akademy -- Frisian$\rightarrow$Dutch} \\
\hline 
...
\hline
\multicolumn{7}{c}{Fryske Akademy -- Dutch$\rightarrow$Frisian} \\
...
l2r1 (experts/replace/opt/ib1) & 0.5155 & 0.6864 & 9.911 & 33.05 & 27.65 \\ 
l2r2 (monolithic/replace/opt/tribl2) & 0.513 & 0.6841 & 9.8885 & 33.28 & 27.84 \\ 
...
We see clear positive results on the EMEA corpus (Table~\ref{tab:emea}), and by
a smaller margin also on the JRC Acquis corpus (Table~\ref{tab:jrc}). Both
experiments were done for English to Spanish, and an English to Dutch
experiment was added for EMEA. In these experiments, we did not conduct
parameter optimisation, we instead used a uniform (uni) distribution of weights for the
translation model.
...
Table~\ref{tab:jrc250k} again clearly illustrates our point that there is
significant variation between data sets; even a different sample from the same
dataset results in very different scores. Nevertheless, the conclusion that
source-side context is beneficial is upheld by both, with the second sample
doing so more by a larger margin. 
...
Figure~\ref{fig:emealc} shows the learning curve for EMEA, English to Spanish, measured along the BLEU metric. All other metrics follow the same trend. Figure~\ref{fig:jrclc} shows the same for JRC Acquis, English to Spanish, here we see BLEU is only about to converge. We therefore also show NIST which converges earlier. For this corpus, the METEOR metric, not shown for brevity, shows a similar late convergence as BLEU, whereas WER and PER confirm trend followed by NIST.

\begin{figure}
\includegraphics[width=180.00mm]{emealcbleu.png}
...
\begin{figure}
\includegraphics[width=90.00mm]{emealcbleu.png}\includegraphics[width=90.00mm]{emealcnist.png}
\caption{Learning curve on JRC Acquis, English to Spanish, measuring BLEU score (left) and NIST (right), logarithmic scale}
\label{fig:emealc}
\end{figure}


In the EMEA learning curve in Table\ref~{fig:emealc}, we included an analysis of different context sizes.
Wider context seems to be beneficial for this data set, but these results have
already been shown not to generalise easily.
...
\subsection{Results: weighting method}
