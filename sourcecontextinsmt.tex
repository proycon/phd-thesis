...
Acquis corpus (Table~\ref{tab:jrc250}) also shows positive results, although by a small
...
ubiquitous in natural language processing, a Zipfian curve can be discerned from
...
kind of optimisation step. The disadvantage is that such systems may 
be more prone to overfitting, which might well be the case in the
EMEA experiment.
% as well as sparsity problems for smaller classifier experts on
%the other hand.
...
($>1$) is of most interest; it covers the phrase pairs for which the
classifier increased the probability.$75\%$ of the
phrase pairs are in this region, which is reflected by the increased
noise in the graph.
...
linguistic features that require supervised parsers or taggers, can
improve translation quality. Memory-based classifiers were used, and
integrated in an SMT framework, following techniques commonly employed
in WSD. The results of the experiments were mixed and there was a high
degree of variability between different corpora and language
pairs. Positive results were attained on only EMEA and JRC-Acquis,
highly formulaic corpora.

We replicated part of the work of \cite{Stroppa+07} and \cite{Rejwanul+11}, and
found that differences in results are fairly marginal. Inclusion of surface-form
...
achieved through optimizing the target-side language model.

We hypothesised that source-side context information may contribute
more when translating to a morphologically more complex language, but
were unable to confirm this hypothesis.
