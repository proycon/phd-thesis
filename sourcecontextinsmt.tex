...
and TRIBL2, a mixture model of the two. These algorithms are implemented in the
...
of a target phrase ($t$) given a source phrase ($s$) and context
information ($c$). We can thus express this as $P(t|s,c)$.
\cite{Stroppa+07} state that direct estimation of this probability using
relative frequencies would result in overestimation of large phrases, and that
therefore a smoothing factor is required. They proceed to say through memory-based
classification we implicitly introduce precisely such a smoothing factor.
