...
$30$ million sentences. These results are shown in Table~\ref{tab:bigbenchmarks}.
...
\hline
Indexed Pattern Model (preloaded), skipgrams, skiptypes$=12$ & 80m 11s & $29746$ MB ($29846$ MB) \\
\hline
...
\end{tabular}
\caption{Benchmarks on two large data sets. Same setup as Table~\ref{tag:benchmarks}.Parameter $y$ represents the occurrence threshold specifically for skipgrams.
}
}
...
\texttt{corpus.txt} and you want to extract all n-grams, skipgrams and flexgrams
...
This has culminated in the publication of several studies that use Colibri Core \cite{COCOCPYP,Kunneman+16}
