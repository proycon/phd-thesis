...
\verb|-no-bos -no-eos|. Note that the encoding of classes is a separate step in
...
\hline
\multicolumn{3}{\textbf{JRC-Acquis data, LM comparison, trigram model ($t=1,l=3$)}}
\hline
Unindexed Pattern Model (from file) & 42 s & $716$ MB & $847$ MB \\
$\ast$ SRILM ngram-count & 12 s & $348$ MB & \\
\hline
\multicolumn{3}{\textbf{Google Billion Words corpus, with heavy thresholding ($t=10,l=4,y=20$)}}
\hline
Unindexed Pattern Pointer Model (preloaded corpus) & 83m 14s & $12279$ MB ($15317$ MB) \\
\end{tabular}
\caption{Benchmarks on two large data sets}
}
