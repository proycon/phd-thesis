...
tasks: Cross-Lingual Word Sense Disambiguation \cite{WSD} and Cross-Lingual
Lexical Substitution \cite{CLLS}. In each task, a number of polysemous words is
...
of the sentence-aligned Europarl parallel corpus \cite{EUROPARL}. This is the
...
exclusively on the Europarl parallel corpus \cite{Europarl}, we select that
...
Tilburg Memory Based Learner (TiMBL) \cite{TIMBL}, making use of the IB1
algorithm, an implementation of the $k$-nearest neighbour classifier. %removed
IB1 cite
...
search algorithm \cite{PARAMSEARCH} determines the optimal set of classifier
parameters for each word expert, including for example the value of $k$ and the
distance weighting metric used.

In the last phase, the classifier output of each word expert is parsed. The
...
as a sentence-aligned parallel corpus. We subsequently run GIZA++ \cite{GIZA}
...
English and Spanish, the software suite Freeling \cite{FREELING} performed all
these tasks, and for Dutch it was done by Tadpole \cite{TADPOLE}, a previous
...
used \cite{PARAMSEARCH}, but optimisation of the feature extraction parameters
...
%In Table \ref{tab:task3trialnl} some of the results on the \emph{trial data} are listed, for the Dutch Word Sense Disambiguation task only. These are precision and recall scores in the first column, and mode precision and recall in the second. All scores are \emph{averaged} over the five target words in the trial data. Included is also a baseline that has been computed by selecting simply the most common translation/sense for each target word, regardless of the context it appears in. 
...
%\hline
%\textbf{Feature configuration} & \textbf{PR} & \textbf{Mode PR} \\
%\hline
%Voter 1) word2+lemma2 2) global & 14.45 & 19.21 \\
...
%\hline
%\end{tabular}
%\caption{Average scores for the English-\emph{Dutch} Word Sense Disambiguation task, averaged over five target words.}
%}
%\end{table}

%Note that at the time this experiment was conducted, a bug existed in the output format specification, resulting in slightly lower scores.

The following two configurations of features were found to perform amongst the
best on the trial data. Therefore they have been selected and submitted for the
contest:
...
both tasks \cite{WSD,CLLS} to refer to our system.
...
\hline
\end{tabular}
\caption{\footnotesize{UvT-WSD1 results in the Cross-Lingual Lexical Substitution task}}
...
\hline
\end{tabular}
\caption{\footnotesize{UvT-WSD1 results in comparison to other participants in the Word-Sense Disambiguation task}}
...
\cite{Hoste+02}, the joint search space of feature selection and
...
\cite{PARAMSEARCH}\footnote{http://ilk.uvt.nl/paramsearch/} was used that
...
words proves beneficial in general, as shown in Table~\ref{tabtrialresults}.
This is therefore the first configuration we submitted (\texttt{c1l}). As
second configuration (\texttt{c1lN}) we submitted the same configuration
without parameter optimisation on the classifiers. Note that neither of these
include global context features.
...
in the last row for reference, this is the HLTDI team \cite{HLTDI} for all but
Best-Spanish, which goes to the NRC contribution \cite{CARPUAT}.
...
Translation system Moses \cite{Koehn+07}, which uses the grow-diag-final
...
or a voter over a set of configurations, for all word experts. The actual
...
takesn parameter optimisation one step further by selecting system parameters
per word expert from the best configurations on the trial data. Optimising the
hyperparameter of the classifiers on the training data proves to have a
slightly negative effect, especially when combined with the selection of
...
An arbiter voting approach over multiple classifiers helps to smooth out errors
...
We can furthermore uphold the conclusion from previous research that including
lemma features is generally a good idea. As to the number of local context
features, we observed that a context size of one feature to the left, and one
to the right, has the best overall average accuracy. Eventually, due to our
feature selection without hyperparameter optimisation on the classifier not
being available yet at the time of submission, our simplest system
\texttt{c1lN} emerged as best in the contest.

When asked to predict the best translation(s), our system comes out on top for
four out of five languages; only for Spanish we are surpassed by two
competitors. Our out-of-five predictions win for two out of five languages, and
are fairly close the the best competitor for the others, except again for
Spanish.   
...
is implemented in Python \cite{PYTHON} and can be obtained from
