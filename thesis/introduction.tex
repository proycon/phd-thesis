...
proper translation. Another example shows where English has one word and French has three
...
This wealth of digital textual data fuels our field of research: Natural
Language Processing (NLP). We attempt to extract meaningful patterns from
natural language data.  Most contemporary techniques in NLP are grounded in
Machine Learning, which employs statistical principles to learn desired
patterns from big collections of natural language data. These data-driven
approaches in NLP can be contrasted to rule-based approaches, not
without their own merit, that are directly derived from human expert knowledge.
...
to efficiently extract patterns such as n-grams, with their context, from corpus data. This software was explicitly
written in the scope of this research project and provides a necessary foundation that is in turn used by most of the
...
assess the feasibility of the idea. It applies the lessons learned from
...
the performance of several 3rd party participating systems on our data. This is set up as a shared task for the SemEval 2014 conference.
...
memory-based classifiers.  Chapter~\ref{chap:conclusion} aggregates and
summarizes the conclusions from earlier chapters, and forms a final answer to
our research questions.

With the exception of Chapter~\ref{chap:contextinsmt}, the other chapters that form the body of this dissertations are
all based on papers that have been published in scientific journals or conference proceedings. Some modifications have
been made to reduce redundancy and fit the over-all narrative of the dissertation, but this has been kept to a minimum.
