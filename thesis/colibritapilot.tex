...
\textsc{This chapter is based on: } \emph{Translation Assistance by Translation of L1 Fragments in an L2 Context}
...
the IB1 algorithm \cite{IB1} as implemented in TiMBL
...
cross-lingual word sense disambiguation as seen in \ref{chap:clwsd}. It has
...
up in a phrase table \cite{Stroppa+07,Haque+11}. The idea of local phrase
selection with a discriminative machine learning classifier using additional
local (source-language) context was introduced in parallel to Stroppa {\em et
al.} \shortcite{Stroppa+07} by Carpuat and Wu \shortcite{Carpuat+07} and
Gim\'{e}nez and M\'{a}rquez \shortcite{Gimenez+07}; cf. Haque {\em et al.}
\shortcite{Haque+11} for an overview of more recent methods.
...
ensemble should be individually optimised \cite{GAMBL}. This we have also shown
